...
- **The system organizes for you.** You don't triage later — you never triage. Keyword search surfaces connections automatically.
- **Status with teeth.** Five states, nothing ambiguous. A task is either backlog, being worked on, stuck, or done.
- **Multi-user with row-level isolation.** Simple username/password auth. Admin sees everything; team members see their own items plus shared projects. Personal projects stay private even from admins.
...
| Layer | Choice | Why |
|---|---|---|
| Framework | Flask + Flask-SQLAlchemy | Known quantity, fast for small-team apps |
| Database | SQLite + FTS5 | Zero infra, keyword search built in, portable |
| Auth | Flask-Login + Werkzeug | Simple username/password, session cookies |
| Frontend templating | Jinja2 | Server-rendered, no build step |
| CSS | Tailwind CSS (via CDN) | Utility-first, fast to build |
...
Access at `http://localhost:5001`. First launch shows the setup wizard to create your admin account.
...
5. Run: `python.exe run.py`

Access at `http://localhost:5001`. First launch shows the setup wizard.
...
- Not enterprise-grade (no OAuth, no SSO, no email invites)
...
*Conceived July 2026. Built for one brain — expanded for teams.*
